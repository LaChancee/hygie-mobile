...
class Question3Page extends StatefulWidget {
  ...
  const Question3Page({Key? key, required this.dependanceType})
      : super(key: key);
...
class _Question3PageState extends State<Question3Page> {
  ...
  String getQuestionText() {
    if (widget.dependanceType == 'tabac') {
      return 'A quelle cigarette renonceriez-vous le plus difficilement ?';
    } else {
      return 'A quelle boisson alcoolisée renonceriez-vous le plus difficilement ?';
    }
  }

  List<String> getOptions() {
    if (widget.dependanceType == 'tabac') {
      return ['A la première de la journée', 'A n\'importe quelle autre'];
    } else {
      return ['A la première de la journée', 'A n\'importe quelle autre'];
    }
  }
                  ...
                  Expanded(
                    child: Padding(
                      ...
                      child: _buildStepIndicator('Questions', true,
                          0.75), // Progression à 75% pour la troisième question
                    ),
                  ),
                  Expanded(
                    child: Padding(
