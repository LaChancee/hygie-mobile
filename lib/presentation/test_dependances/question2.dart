...
class Question2Page extends StatefulWidget {
  ...
  const Question2Page({Key? key, required this.dependanceType})
      : super(key: key);
...
class _Question2PageState extends State<Question2Page> {
  ...
  String getQuestionText() {
    if (widget.dependanceType == 'tabac') {
      return 'Trouvez-vous qu\'il est difficile de vous abstenir de fumer dans les endroits où c\'est interdit ? (ex: cinémas, bibliothèques)';
    } else {
      return 'Trouvez-vous qu\'il est difficile de vous abstenir de boire de l\'alcool dans les endroits où c\'est interdit ?';
    }
  }

  List<String> getOptions() {
    return ['Oui', 'Non']; // Options simples binaires pour cette question
  }
                  ...
                  Expanded(
                    child: Padding(
                      ...
                      child: _buildStepIndicator('Questions', true,
                          0.5), // Progression à 50% pour la deuxième question
                    ),
                  ),
                  Expanded(
                    child: Padding(
